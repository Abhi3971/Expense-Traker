...
const HomePage = () => {
  ...
  const handleDelete = async (record) => {
    try {
      ...
      setAllTransection((prevTransection) =>
        prevTransection.filter((transection) => transection._id !== record._id)
      );
    } catch (error) {
      setLoading(false);
      console.log(error);
  ...
  // Form handling for adding or editing transactions
  const handleSubmit = async (values) => {
    try {
      ...
      if (editable) {
        ...
        setAllTransection((prevTransection) => [
          ...prevTransection,
          res.data, // Assuming `res.data` contains the newly added transaction
        ]);
      }
      setShowModal(false);
      setEditable(null);
